...
const TutorialModal = ({ isOpen, onClose, tutorialId = null, onSuccess }: TutorialModalProps) => {
  ...
  const { data: tutorialData, isLoading: isLoadingTutorial } = useGetSingleTutorialQuery(tutorialId || "", { skip: !tutorialId });
  ...
  useEffect(() => {
    if (tutorialData?.data && isEditMode) {
  ...
  }, [tutorialData, isEditMode, isOpen]);
  ...
  const convertToEmbedUrl = (url: string): string => {
    ...
    // If no match, return original URL (might be already correct or invalid)
    return url;
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // Ensure YouTube URL is in embed format before submitting
    const embedUrl = convertToEmbedUrl(formData.youtubeUrl);
    ...
    try {
      ...
      if (isEditMode && tutorialId) {
        await updateTutorial({ id: tutorialId, ...tutorialData }).unwrap();
  ...
  const categories = ["Recommendation", "Getting Started", "Discovery", "Reviews", "Goals", "Tips", "Community"];
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .tutorial-modal-overlay { animation: fadeIn 0.2s ease-out forwards; }
        .tutorial-modal-content { animation: fadeIn 0.3s ease-out forwards; }
        ...
        >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              ...
              <div>
                ...
                <p style={{ color: "rgba(255, 255, 255, 0.4)", fontSize: "14px", marginTop: "4px" }}>
                  {isEditMode ? "Update tutorial information" : "Add a new tutorial video"}
                </p>
              </div>
            </div>
          </div>
          ...
          {isEditMode && isLoadingTutorial ? (
            <div style={{ padding: "60px", textAlign: "center" }}>
              ...
              <p style={{ color: "rgba(255, 255, 255, 0.5)", fontSize: "14px" }}>Loading tutorial data...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} style={{ padding: isMobile ? "24px" : "32px" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
                ...
                {/* YouTube URL */}
                <div>
                  <label style={labelStyle}>YouTube URL <span style={{ color: "#ef4444" }}>*</span></label>
                  <input
                    type="url"
                    name="youtubeUrl"
                    ...
                    onChange={handleChange}
                    required
                    placeholder="https://www.youtube.com/watch?v=abcd1234 or https://youtu.be/abcd1234"
                    style={inputStyle}
                    onFocus={(e) => e.currentTarget.style.borderColor = "rgba(220, 38, 38, 0.5)"}
                    onBlur={(e) => e.currentTarget.style.borderColor = "rgba(255, 255, 255, 0.1)"}
                  />
                  <p style={{ color: "rgba(255, 255, 255, 0.3)", fontSize: "12px", marginTop: "8px" }}>
                    Paste any YouTube URL (watch, youtu.be, or embed format) - it will be automatically converted
                  </p>
                </div>
                ...
                {formData.youtubeUrl && (
                  <div>
                    ...
                    <div style={{ position: "relative", paddingTop: "56.25%", background: "rgba(0, 0, 0, 0.3)", borderRadius: "12px", overflow: "hidden", border: "1px solid rgba(255, 255, 255, 0.1)" }}>
                      <iframe
